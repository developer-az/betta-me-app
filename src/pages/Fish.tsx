      ...
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
        <div className="flex flex-col items-center justify-center gap-6">
          ...
          <motion.div animate={{ y: [0, -10, 0, 10, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}>
            <FishSVG color={fish?.color || '#e57373'} mood="happy" />
          </motion.div>
          <div className="w-full max-w-xl rounded-2xl bg-slate-50/80 dark:bg-slate-800/70 shadow p-5 space-y-6">
            ...
            <div>
              ...
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 mb-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.finCondition || 'Healthy'} onChange={e => setFish?.({ ...fish!, finCondition: e.target.value })}>
              ...
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 mb-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.colorCondition || 'Vibrant'} onChange={e => setFish?.({ ...fish!, colorCondition: e.target.value })}>
                <option>Vibrant</option>
                <option>Faded</option>
                <option>Spots</option>
              </select>
              <label className="block text-sm font-semibold mb-1">Gills/Breathing</label>
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 mb-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.gillCondition || 'Normal'} onChange={e => setFish?.({ ...fish!, gillCondition: e.target.value })}>
                <option>Normal</option>
                <option>Rapid</option>
                <option>Gasping</option>
              </select>
              <label className="block text-sm font-semibold mb-1">Body Condition</label>
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.bodyCondition || 'Normal'} onChange={e => setFish?.({ ...fish!, bodyCondition: e.target.value })}>
                <option>Normal</option>
            ...
            <div>
              ...
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 mb-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.appetite || 'Eating well'} onChange={e => setFish?.({ ...fish!, appetite: e.target.value })}>
                <option>Eating well</option>
                <option>Not eating</option>
              </select>
              <label className="block text-sm font-semibold mb-1">Activity</label>
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 mb-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.activity || 'Normal'} onChange={e => setFish?.({ ...fish!, activity: e.target.value })}>
                <option>Normal</option>
                <option>Lethargic</option>
                <option>Hyperactive</option>
              </select>
              <label className="block text-sm font-semibold mb-1">Behavior</label>
              <select className="w-full border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" value={fish?.behavior || 'Normal'} onChange={e => setFish?.({ ...fish!, behavior: e.target.value })}>
                <option>Normal</option>
